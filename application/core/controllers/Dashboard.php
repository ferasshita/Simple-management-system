	...
	public function user(){
		...
		if (isset($_POST['submit_uInfo'])) {
			...
			if ($stop != "1") {
				...
				$update = "UPDATE signup SET Username = '$db_username',Email = '$db_email',package = '$package',admin = '$db_admin' WHERE Username = '$ed'";
				}else{
				$update = "UPDATE signup SET Username = '$db_username',Email = '$db_email',package = '$package',Password = '$db_password',admin = '$db_admin' WHERE Username = '$ed' ";
				}
		...
		if (isset($_POST['susbend'])) {
			$uInfo = "SELECT sus FROM signup WHERE Username = '$ed'";
			...
			$query ="UPDATE signup SET sus = '$sus' WHERE Username = '$ed' ";
		...
		$ed = trim(filter_var(htmlspecialchars($_GET['ed']),FILTER_SANITIZE_STRING));
		 $uInfo = "SELECT id,Username,Email,phone,Password,admin,nex_pay,sus,accou_stu,package,package_chose FROM signup WHERE Username =  '$ed' ";
		$FetchedData=$this->comman_model->get_all_data_by_query($uInfo);
		...
		if (isset($_POST['rAccBtn'])) {
			...
			{
				//echo "Test";exit;
				echo $url=base_url()."Dashboard/panel?adb=users";
				header("location: $url");
				exit;
			}
			else
		...
		{
			$admin_int = "1";
			$chAdmin = "SELECT Username FROM signup WHERE Username = '$ed' AND admin = '$admin_int' ";
