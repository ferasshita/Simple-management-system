...
function exch_rat(){
    var cContent = $.trim($('#exch_input').val());
    var path = "<?php echo $checkDir; ?>";
    $('#prus').val(cContent);
    $.ajax({
        type:'POST',
        url:'<?php echo base_url(); ?>Dashboard/exchange_rate',
...
function delvgj(){
  if(confirm("Are you sure to delete your account permently?")){
    if(confirm("You will lose your data?")){
      if(confirm("You can't recover your account?")){
      }else{
        return false;
      }
    }else{
...
function deleteban(cid){
  if(confirm("<?php echo lang('are_delete'); ?>")){
    $.ajax({
        type:'POST',
        url:'<?php echo base_url(); ?>Setting/delete_cos_bank',
        data:{'cid':cid},
        beforeSend: function(){
            $('#tr_'+cid).hide();
        },
...
function deletebanor(cid){
  if(confirm("<?php echo lang('are_delete'); ?>?")){
    $.ajax({
        type:'POST',
        url:'<?php echo base_url(); ?>Dashboard/delete_my_bank',
        data:{'cid':cid},
        beforeSend: function(){
            $('#tr_'+cid).hide();
        },
...
function deleteexf(cid){
  if(confirm("<?php echo lang('are_delete'); ?>")){
    $.ajax({
        type:'POST',
        url:'<?php echo base_url(); ?>Expenses/delete_expenses',
        data:{'cid':cid},
        beforeSend: function(){
            $('#tr_'+cid).hide();
        },
...
window.open('<?php echo base_url(); ?>Transaction/print?pid='+cid);

}
function package_chose(cid){
  if(confirm("<?php echo lang('are_package'); ?>")){
    $.ajax({
        type:'POST',
        url:'<?php echo base_url(); ?>Packages/Updatepackage',
        data:{'cid':cid},
        beforeSend: function(){
          $('#package_loader_'+cid).show();
            $('#package_text_'+cid).hide();
        },
        success: function(done){
            if (done == 'yes') {
              $('#package_loader_'+cid).hide();
              $('#package_text_'+cid).show();
              $('#pack_btn_'+cid).css({"background":"tomato"});
                $('#pack_btn_<?php echo $_SESSION['package_chose']; ?>').css({"background":"#0080ff"});
            }else{
              $('#package_loader_'+cid).hide();
                $('#package_text_'+cid).show();
                alert('Action denied! You are not allowed to doing this action');
...
function deletetr(cid){
  if(confirm("<?php echo lang('are_delete_per'); ?>")){
    $.ajax({
        type:'POST',
        url:'<?php echo base_url(); ?>Trash/delete_perment',
        data:{'cid':cid},
        beforeSend: function(){
            $('#tr_'+cid).hide();
        },
...
function ovar(cid){
$('#fgd_'+cid).slideToggle(300);
}
function suover(cid){
var cContent = $.trim($('#hhf_'+cid).val());
if (cContent == '') {

}else{
$.ajax({
type: "POST",
url: "Transaction/pay_invest",
data: { "cid":cid,"cContent":cContent },
cache: false,
dataType: "json",
beforeSend: function(loading){
$('#fgd_'+cid).hide();
...
function chehu(cid){
  if(confirm("<?php echo lang('are_finish'); ?>")){
    $.ajax({
        type:'POST',
        url:'<?php echo $checkDir; ?>Transaction/close_invest',
        data:{'cid':cid},
        beforeSend: function(){
            $('#ok_'+cid).hide();
        },
...
function finish(cid){
  if(confirm("<?php echo lang('are_finish'); ?>")){
    $.ajax({
        type:'POST',
        url:'<?php echo base_url(); ?>Transaction/finish_transaction',
        data:{'cid':cid},
        beforeSend: function(){
            $('#okf_'+cid).hide();
        },
...
function deletere(cid){
  if(confirm("<?php echo lang('are_revive'); ?>")){
    $.ajax({
        type:'POST',
        url:'<?php echo base_url(); ?>Transaction/revive_transaction',
        data:{'cid':cid},
        beforeSend: function(){
            $('#tr_'+cid).hide();
        },
        success: function(done){
            if (done == 'yes') {
                ...
                $("#mod").load(location.href + " #mod");
                $("#mcod").load(location.href + " #mcod");
                $("#refrish").load(location.href + " #refrish");
...
function deletepro(cid){
  if(confirm("<?php echo lang('are_delete'); ?>")){
    $.ajax({
        type:'POST',
        url:'<?php echo base_url(); ?>Wallet/delete_wallet',
        data:{'cid':cid},
        beforeSend: function(){
            $('#tr_'+cid).hide();
        },
...
function deletear(cid){
  if(confirm("<?php echo lang('are_delete'); ?>")){
    $.ajax({
        type:'POST',
        url:'<?php echo base_url(); ?>Transaction/delete_transaction',
        data:{'cid':cid},
        beforeSend: function(){
            $('#tr_'+cid).hide();
        },
...
$.ajax({
type: "POST",
url: "<?php echo $checkDir; ?>includes/pay_invest.php",
...
$.ajax({
    type:'POST',
    url:'<?php echo $checkDir; ?>includes/delete_perment.php',
    data:{'pid':pid},
    beforeSend: function(){
    $('#'+pid).hide();
    },
...
$.ajax({
    type:'POST',
    url:'<?php echo $checkDir; ?>includes/delete_transaction.php',
    data:{'cid':cid},
    beforeSend: function(){
        $('#comment_'+cid).hide();
    },
...
$.ajax({
    url:"<?php echo $check_path; ?>includes/revive_transaction.php",
    type:"POST",
    data:"sid="+sid,
    beforeSend:function(){
        $("#saved_"+sid).hide();
